...
| 기능 | Goodnotes | Notability | 현재 StudyCoach `0.1.19` 대응 |
|---|---|---|---|
...
| 도구별 색상 | 지원 | 지원 | 🟨 시스템 도구 UI에서만 설정 가능; 앱 자체 프리셋 없음 |
| 도구별 굵기 | 지원 | 지원 | 🐞 시스템 UI는 있으나 사용자가 요구한 더 얇은 최소 굵기 해결 여부 미확정 |
| 필압·기울기 반응 | 지원 | 지원 | ✅ PaperKit 시스템 필기 엔진이 담당; 앱이 획을 직접 합성하지 않음 |
| 획 전체 지우개 | 지원 | 지원 | 🧪 시스템 지우개가 작동했던 이력은 있으나 `0.1.19` 전체 회귀 필요 |
| 부분/정밀 지우개 | 지원 | 지원 | 🧪 시스템 지우개 옵션의 현재 동작 재검증 필요 |
...
| 실행 취소·다시 실행 | 지원 | 지원 | 🟨 시스템 undo manager는 있으나 현재 PDF 시험 화면에 전용 버튼 없음 |
| Apple Pencil 두 번 탭 | 지원 | 지원 | 🧪 활성 시스템 도구 선택기가 처리할 수 있으나 현재 회귀 시험 필요 |
...
| 레이저 포인터 | 지원 | 지원 | ❌ 없음 |
| 자체 디자인 필기 툴바 | 지원 | 지원 | ❌ 현재 Apple 기본 도구 선택기 사용 |
...
이 문서는 기능 존재 여부만 기록한다. 자체 툴바의 상태 구조, PDF 페이지
기하 구조, 구현 순서와 버전 계획은 다음 설계 작업에서 별도로 정한다.
